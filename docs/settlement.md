...
<figure class="mechanism-video">
  <video controls muted playsinline preload="metadata" poster="/mechanisms/push-solver-scoring-board-only-poster.jpg" aria-label="Focused Zonoid bounded settlement demo board recording">
    <source src="/mechanisms/push-solver-scoring-board-only.mp4" type="video/mp4">
    Your browser does not support embedded video.
  </video>
  <figcaption>Focused Zonoid demo board: the push and objective arrival settle in one bounded deterministic turn pipeline.</figcaption>
</figure>
